...
func _ready():
	...
	if caveentrance != null:
		var entrance_pos = caveentrance[0]
		var target_pos = Vector3(entrance_pos.x, entrance_pos.y + 100, entrance_pos.z)
		if player != null:
			player.global_transform = Transform3D(player.global_transform.basis, target_pos)
		else:
			var char_node = get_parent().get_node_or_null("CharacterBody3D")
			if char_node != null:
				char_node.global_transform = Transform3D(char_node.global_transform.basis, target_pos)
			else:
				push_warning("characterbody3dが見つかりません。プレイヤーをcaveentranceに移動できません")
	else:
		push_warning("caveentranceがnullです。")
	var px
	var py
	...
	var mesh_node = get_parent().get_node_or_null("StaticBody3D/MeshInstance3D")
	if mesh_node == null:
		mesh_node = find_mesh_instance(get_parent())
	if mesh_node != null and mesh_node is MeshInstance3D and mesh_node.mesh != null:
		template_mesh = mesh_node.mesh
	else:
		push_warning("template meshが見つかりません。代用のcubemeshを使用します")
...
=======
>>>>>>> 3dc00c090bbf402c93d11f7f91316e86de1b4521
